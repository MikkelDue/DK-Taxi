<template>
  ...
  <div id="amelia-container" ref="ameliaContainer" class="am-fs__wrapper" :style="cssVars">
    <SideBar v-if="sidebarVisibility && containerWidth > 560" class="am-fs-sb">
      <template #step-list>
        <div class="am-fs-sb__step-wrapper">
          <template v-if="stepsArray[stepIndex] !== congratulationsStep">
            ...
            >
              ...
              <TransitionGroup tag="span" name="fade" appear class="am-fs-sb__step-selection__wrapper">
                <p
                  v-for="itemSelected in step.stepSelectedData" :key="itemSelected.position"
          ...
          <template v-else>
            <AddToCalendar :use-in-sb="true" :calendar-string="labelsDisplay('add_to_calendar', 'congratulations')"></AddToCalendar>
          </template>
        </div>
      </template>
...
let amFonts = amCustomize.value.fonts

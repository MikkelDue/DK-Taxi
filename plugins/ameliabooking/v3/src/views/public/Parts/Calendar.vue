...
import { useAvailableSlots, useCalendarEvents } from "../../../assets/js/common/appointments.js";
...
function getSlotsCallback (slots, minimumDateTime, maximumDateTime, customCallback) {
  ...
  if (cartItem.value.index !== '') {
    ...
    } else if (!(activeService.list[cartItem.value.index].time in slots[activeService.list[cartItem.value.index].date])) {
      store.commit('booking/setMultipleAppointmentsTime', null)

      calendarEventSlot.value = ''
    }

    if (activeService.list[cartItem.value.index].date &&

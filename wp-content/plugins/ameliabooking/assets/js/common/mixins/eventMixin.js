...
export default {
  methods: {
    ...
    getMaxTicketSelected(evt, ticket, bookingToEventTickets) {
      if (evt.bringingAnyone) {
        if (evt.maxCustomCapacity) {
          ...
          return evt.maxCustomCapacity - this.getAllSoldTickets(evt) - selected
        }
        return ticket.spots - ticket.sold
      }
      return 1
    },
    ...
    setTicketsData (event) {
      ...
      let max = event.maxCustomCapacity
      let sold = this.getAllSoldTickets(event)
      ...
      event.customTickets.forEach((ticket, index) => {
        ...
        if (!enabledFirst && ticket.enabled && (max ? (max - sold > 0) : (ticket.spots - ticket.sold > 0))) {
          enabledFirst = true
        }
      })

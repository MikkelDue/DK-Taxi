<template>
  <div class="am-wrap">
    <transition name="fade">
      <div v-if="promoBannerVisible && $root.settings.activation.enableAmeliaPromoBanner" class="am-promo-banner">
        <div class="am-promo-banner__content">
          <div class="am-promo-banner-left">
            ...
            <div class="am-upgrade-to-premium">
              <p class="am-amelia-text">Amelia</p>
              <p>Upgrade to Premium today!</p>
            </div>

          </div>
          <div class="am-promo-banner-center">
            <img :src="$root.getUrl + 'public/img/promo-discount-30-1.svg'" alt="Discount 30">
          ...
          <div class="am-promo-banner__buttons">
            ...
            <p @click="removeBanner">{{ $root.labels.not_show }}<img :src="$root.getUrl + 'public/img/am-close-x.svg'" alt="Close"></p>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      if (localStorage.getItem('ameliaPromoBanner') !== null) {
        this.promoBannerVisible = JSON.parse(localStorage.getItem('ameliaPromoBanner'))
      }
    },
    ...
    methods: {
      ...
      removeBanner () {
        localStorage.setItem('ameliaPromoBanner', false)

        this.$http.post(`${this.$root.getAjaxUrl}/settings`, {activation: {enableAmeliaPromoBanner: !this.promoBannerVisible}})
          .then(response => {
          })
          .catch(e => {
          })

        this.promoBannerVisible = false
      },
...
.am-promo-banner {
  ...
  margin-bottom: 20px;
  border-radius: 20px;
  background: linear-gradient(270deg, #092272 15.35%, #030912 100%);
  line-height: 0;

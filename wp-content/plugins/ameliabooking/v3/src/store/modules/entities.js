...
export default {
  ...
  getters: {
    ...
    filteredPackages: (state, getters) => (data) => {
      return state.packages.filter(pack =>
          pack.status === 'visible' && pack.available &&
          (!data.serviceId ? true :
    ...
    filteredLocations: (state, getters) => (data) => {
      return state.locations.filter(location =>
        (!data.providerId ? true :
          state.employees.find(i => i.id === data.providerId).serviceList.filter(
            employeeService => {
    ...
    getEmployeeServices: (state, getters) => (data) => {
      ...
      if (data.serviceId) {
        ...
        if (data.providerId) {
          let employeeService = state.employees.find(i => i.id === data.providerId).serviceList.filter(service => service.id === data.serviceId)
          return employeeService.map(eS => Object.assign(eS, {bringingAnyone: service.bringingAnyone, aggregatedPrice: service.aggregatedPrice}))
        }
        ...
        getters.filteredEmployees(data).forEach((employee) => {
          employee.serviceList.forEach((employeeService) => {
            if (employeeService.id === data.serviceId) {
              employeeServices.push(Object.assign(employeeService, {bringingAnyone: service.bringingAnyone, aggregatedPrice: service.aggregatedPrice}))
            }
          })
        })
      }
  ...
  mutations: {
    ...
    setPreselectedValues (state) {
      state.services = state.services.filter(s => s.status === 'visible' && s.show)
    ...
    getEntities ({ commit }, payload) {
      ...
      if (payload.loadEntities && !getEntitiesVariableName()) {
        httpClient.get('/entities', { params: useUrlParams({ types }) }).then(response => {
          ...
          setEntities({ commit }, response.data.data, types)
        })
      } else {
        let ameliaApiInterval = setInterval(
          () => {
            ...
            if (name) {
              ...
              setEntities({ commit }, window[name], types)
            }
          },
          1000
        )

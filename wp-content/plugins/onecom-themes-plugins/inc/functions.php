...
if( ! function_exists( 'onecom_themes_cat_count' ) ) {
	function onecom_themes_cat_count() {
		...
		if (is_array($themes)){
			...
			$exclude_themes = array('the-anderson-family', 'summertime-adventure', 'school-days', 'personal-cv', 'heisengard', 'gardener', 'onecom-ilotheme');
			foreach($themes as  $theme_group){
				if (isset($theme_group->collection)){
...
if( ! function_exists( 'onecom_plugins_count' ) ) {
	function onecom_plugins_count() {
		...
		//echo "<pre>";
		//print_r(onecom_fetch_plugins());
		if (is_array($oc_plugins)) {
			$plugins['onecom'] = count($oc_plugins);
			// @todo - recount to -1 once onephoto removed permanantely from json
			$plugins['onecom_excluding_generic'] = count($oc_plugins)-2;
		} else {
			$plugins['onecom'] = '';
